...
class RuntimeIdentityCredentialIssuerTest {
    ...
    void shouldIssueFreshCredentialsForClient() {
        ...
        when(identityRefreshTokenManager.issue(1L, IdentityClientType.MOBILE)).thenReturn("refresh-token");
        when(userRepository.save(user)).thenReturn(user);
        when(jwtTokenProvider.generateAccessToken(eq(1L), eq("alice"), anyString(), eq(IdentityClientType.MOBILE)))
                .thenReturn("access-token");

        IssuedAuthCredentials issued = issuer.issueFresh(user.getId(), IdentityClientType.MOBILE);
        ...
        assertThat(issued.user().id()).isEqualTo(user.getId());
    }
    ...
    void shouldIssueCredentialsWithProvidedRefreshToken() {
        ...
        IssuedAuthCredentials issued = issuer.issueWithRefreshToken(user.getId(), "provided-refresh", IdentityClientType.DESKTOP);
