...
public class StoragePolicyService implements CommandLineRunner, StoragePolicyQuery {
    ...
    public DefaultStoragePolicySnapshot readDefaultPolicySnapshot() {
        ...
        return new DefaultStoragePolicySnapshot(policy.getId(), policy.getMaxSizeBytes(), readCapabilities(policy));
    }

    @Override
    public StoragePolicyDescriptor readPolicyDescriptor(Long policyId) {
        StoragePolicy policy = getRequiredPolicy(policyId);
    ...
    @Override
    public StoragePolicyCapabilities readPolicyCapabilities(Long policyId) {
        return readCapabilities(getRequiredPolicy(policyId));
    }
    ...
    public StoragePolicy getRequiredPolicy(Long policyId) {
        return storagePolicyRepository.findById(policyId)
                .orElseThrow(() -> new BusinessException(ErrorCode.STORAGE_POLICY_NOT_FOUND, "存储策略不存在"));
    }
