...
public class RuntimeAdminConfigSchemaApi implements AdminConfigSchemaApi {
    ...
    private final AdminConfigValueGovernanceService adminConfigValueGovernanceService;
    ...
    public AdminConfigDefinitionResponse updateValue(String key, AdminConfigUpdateRequest request) {
        return adminConfigValueGovernanceService.updateValue(key, request);
    }

    @Override
    public PageResponse<AdminConfigHistoryResponse> history(String key, int page, int size) {
        return adminConfigValueGovernanceService.history(key, page, size);
    }

    @Override
    public AdminConfigDefinitionResponse rollback(String key, long version) {
        return adminConfigValueGovernanceService.rollback(key, version);
    }
